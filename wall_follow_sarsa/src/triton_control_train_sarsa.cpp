...
vector<vector<double>> update_q_table(vector<vector<double>> qVals, int current_state, int new_state, int current_action, int new_action, int nActions, double theta_err);
...
int main(int argc, char **argv)
{
    ...
    t1 =ros::Time::now().toSec();
    int current_state, current_action, new_state, new_action;
    ...
    while (t1-t0 < total_time)
    {
        ...
        // ***FIND NEW ACTION***
        // Find what action robot should take using epsilon-greedy policy
        new_action = find_action_e_greedy(qVals, nActions, new_state);

        // ***UPDATE Q-TABLE***
        qVals = update_q_table(qVals, current_state, new_state, current_action, new_action, nActions, theta_err);
...
vector<vector<double>> update_q_table(vector<vector<double>> qVals, int current_state, int new_state, int current_action, int new_action, int nActions, double theta_err){
    ...
    double reward = 180-abs(theta_err)*180/PI; // Reword defined as the angle difference between the robots heading and where it's heading should be

    qVals[current_state][current_action] = qVals[current_state][current_action] + learn_rate * (reward + relative_value*qVals[new_state][new_action] - qVals[current_state][current_action]);
